...
const LavaLamp: React.FC = () => {
    ...
    morphScript.onload = async () => {

        const blobs = document.querySelectorAll('#blobs .morphing-blob');


        blobs.forEach(
            blob => {
                const blobElement = blob as HTMLElement; // Cast to HTMLElement
                const morphShapes: NodeList = blob.querySelectorAll('path');

                const path1: string = '#' + (morphShapes[0] as HTMLElement).getAttribute('id');
                const path2: string = '#' + (morphShapes[1] as HTMLElement).getAttribute('id');
                ...
                KUTE.fromTo(
                    ...
                    {path: path1},
                    {path: path2},
                    {repeat: 999, duration: blob === blobs[0] ? 5000 : morphDuration, yoyo: true}
                ).start();

                if (blobs[0] === blob)// first blob behaviour is static(not randomized)
                    return

                //randomization
                blobElement.style.transform = 'scale(' + (Math.round(Math.random() * 80) + 20) + '%)';
                blobElement.style.left = Math.round(Math.random() * 100) + '%';
                blobElement.style.animationDelay = Math.round(Math.random() * 10) + 's';
                blobElement.style.animationDuration = Math.round(Math.random() * 8 + 22) + 's';


                blobElement.addEventListener('animationiteration', event => {
                    if (event.animationName === 'blob-float') {
                        blobElement.style.left = Math.round(Math.random() * 100) + '%';
                        blobElement.style.transform = 'scale(' + Math.round(Math.random() * 80) + 20 + '%)';
    ...
    return (
        <div>
            ...
            <div id="blobs">
                ...
                <div className="morphing-blob randomized">
                    <svg
                        id="visual"
                        viewBox="0 0 900 600"
                        width="900"
